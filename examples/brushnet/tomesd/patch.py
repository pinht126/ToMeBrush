...
def compute_merge(x: torch.Tensor, tome_info: Dict[str, Any]) -> Tuple[Callable, ...]:
    ...
    if downsample <= args["max_downsample"]:
        ...
        #print(mask)
        if mask:
            m, u = merge.bipartite_soft_matching_mask_random2d(x, w, h, args["sx"], args["sy"], args["ratio1"], args["ratio2"],
                                                          no_rand=not use_rand, mask_image=args['mask_image'], generator=args["generator"])
        else:
            m, u = merge.bipartite_soft_matching_random2d(x, w, h, args["sx"], args["sy"], args["ratio1"],
...
def apply_patch(
    ...
    if mask_path is not None:
        ...
        mask_image = torch.from_numpy(mask_path).permute(2, 0, 1)
        mask_image = mask_image.unsqueeze(0)
        mask_image = torch.nn.functional.interpolate(mask_image, size=(64, 64))
        mask_image = mask_image.squeeze(0).squeeze(0)
    else:
        mask_image = torch.ones((256, 256), dtype=torch.float64)
